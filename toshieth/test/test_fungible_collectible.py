...
class ERC721Test(EthServiceBaseTest):
    ...
    async def test_createarttoken(self, *, parity, push_client, monitor, collectible_monitor):
        ...
        # "mint" some tokens
        txhash = await creator_contract.createAsset.set_sender(TEST_PRIVATE_KEY)("ART", 10, "dasdasdasdasdasdasdasd", TEST_ADDRESS)
        ...
        self.assertEqual(await arttoken1.name(), "ART")
        ...
        async with self.pool.acquire() as con:
            owner_balance = await con.fetchval(
                "SELECT balance FROM fungible_collectible_balances WHERE owner_address = $1",
                TEST_ADDRESS)
        ...
        self.assertEqual(body['collectibles'][0]['value'], hex(1))

        resp = await self.fetch("/collectibles/{}/{}".format(TEST_ADDRESS, creator_contract.address))
        self.assertResponseCodeEqual(resp, 200)
        body = json_decode(resp.body)
        self.assertEqual(len(body['tokens']), 1)
        self.assertEqual(body['tokens'][0]['image'], "https://ipfs.node/dasdasdasdasdasdasdasd")
